...
let add_exception_handling w (fns: Typed_cmm.func_result list) = (
    ...
    let func (f: Ast.func) = (
         let get_local_position name = (
            ...
            find 0 f.locals )
        in
        let rec fix_body handle_exception type_ exception_depth result (il:Ast.instr list) = (
            match il with
            | Loop (t, e) :: remaining -> 
                fix_body handle_exception type_ exception_depth (result @ [Loop (t, fix_body handle_exception type_ (Int32.add exception_depth 1l) [] e)]) remaining                
            | Block (t, e) :: remaining -> 
                fix_body handle_exception type_ exception_depth (result @ [Block (t, fix_body handle_exception type_ (Int32.add exception_depth 1l) [] e)]) remaining
            | If (t, e1, e2) :: remaining -> 
                fix_body handle_exception type_ exception_depth (result @ [If (t, fix_body handle_exception type_ (Int32.add exception_depth 1l) [] e1, fix_body handle_exception type_ (Int32.add exception_depth 1l) [] e2)]) remaining                
            | TryCatch (s, then_, exception_name, catch_) :: remaining ->
                ...
                let result = fix_body handle_exception type_ exception_depth (result @ 
                    [Block (s, 
                        [Block (
                            [],
                            (
                        ...
                        @
                        (fix_body handle_exception type_ 0l [] catch_)
                    )]
                ) remaining in
                result
            | CallIndirect (t, args) :: remaining -> 
                ...
                | None -> [I32Type]
                in
                fix_body handle_exception type_ exception_depth (result @ [
                    CallIndirect (t, List.map (fun f -> fix_body handle_exception type_ exception_depth [] f) args);
                    DataSymbol "_exception_thrown";
                    Load {ty = I32Type; align = 0; offset = 0l; sz = None};
                    If ([], 
                        (
            ...
            | Call (function_name, args) :: remaining -> (
                ...
                in
                fix_body handle_exception type_ exception_depth (result @ [
                    Call (function_name, List.map (fun f -> fix_body handle_exception type_ exception_depth [] f) args);
                    DataSymbol "_exception_thrown";
                    Load {ty = I32Type; align = 0; offset = 0l; sz = None};
                    If ([], 
                        (
            ...
            | other :: remaining -> 
                fix_body handle_exception type_ exception_depth (result @ [other]) remaining
            | [] -> result   
        )
        in
        {f with           
          body = fix_body false [] 0l [] f.Ast.body
        }  
    )
    in
    (Ast.{w with 
