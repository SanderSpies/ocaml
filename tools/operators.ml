...
let i32_const n = Const (I32 n)
let i64_const n = Const (I64 n)
let f32_const n = Const (F32 n)
let f64_const n = Const (F64 n)
...
let local_get x = GetLocal x
let local_set x = SetLocal x
let local_tee x = TeeLocal x
let global_get x = GetGlobal x
let global_set x = SetGlobal x
...
let memory_size = CurrentMemory
let memory_grow = GrowMemory
